...
The actual production database uses a table called `creator_profiles` (see WR_MemberDBSchema.octet-stream for full details). Our mock schema adapts this structure for POC development with some simplifications and additions specific to the Profile Optimizer agents.

### Membership Form Fields vs Provided Schema

**Fields Present in Membership Form:**

| Form Section | Field | In `creator_profiles`? | Where is it stored? |
|--------------|-------|------------------------|---------------------|
| Basic Info | Profile photo | ✅ `profile_image_url` | creator_profiles |
| Basic Info | First name | ✅ `first_name` | creator_profiles |
| Basic Info | Last name | ✅ `last_name` | creator_profiles |
| Basic Info | What you do | ✅ `bio` or `role` | creator_profiles |
| Basic Info | Where (location) | ✅ `location` | creator_profiles |
| Links & Contact | Phone number | ❌ **MISSING** | **Unknown** |
| Links & Contact | Website | ✅ `website` | creator_profiles |
| Links & Contact | Social links | ✅ `social_links` (JSONB) | creator_profiles |
| Traits | Skills | ❌ Not in provided schema | `creator_traits`? |
| Traits | Interests | ❌ Not in provided schema | `creator_traits`? |
| Traits | Inspirations | ❌ Not in provided schema | `creator_traits`? |
| Prompts | Current focus | ❌ Not in provided schema | `prompt_responses`? |
| Prompts | Needs for vision | ❌ Not in provided schema | `prompt_responses`? |
| Prompts | Motivations | ❌ Not in provided schema | `prompt_responses`? |

**Summary:** Only 7 out of 13 membership form fields are documented in the provided schema. We need schemas for `prompt_responses` and `creator_traits` tables to complete the picture.

### Key Differences Between Production and Mock Schema

**Production Schema (`creator_profiles` table):**
- Uses Clerk authentication (`clerk_user_id`, `clerk_email`)
- Includes membership tiers and status tracking
- Has `bio` field (single text field)
- Uses `social_links` as JSONB array of strings
- Includes `roles` array and `company` fields
- Has complex membership status enum with 15+ values
- Includes payment/subscription tracking fields

**Mock Schema (POC):**
- Simplified authentication (no Clerk integration for POC)
- Focuses on profile optimization features only
- Separates traits into normalized tables (skills, interests, inspirations)
- Uses separate `social_links` table with processing status tracking
- Includes `prompt_responses` table for long-form answers
- Adds `conversation_history` for agent interactions
- Adds `profile_completeness` for assessment caching
- Removes membership/payment complexity for POC

### Mapping Production to Mock Schema

| Production Field | Mock Equivalent | Notes |
|------------------|-----------------|-------|
| `id` | `members.id` | Direct mapping |
| `clerk_user_id` | Not in mock | Authentication simplified for POC |
| `clerk_email` | Not in mock | Authentication simplified for POC |
| `first_name` | `members.first_name` | Direct mapping |
| `last_name` | `members.last_name` | Direct mapping |
| `bio` | `members.what_you_do` | Renamed for clarity |
| `urls` | Not in mock | Replaced by structured `social_links` table |
| `roles` | Not in mock | Could map to skills/interests |
| `company` | Not in mock | Could add if needed |
| `role` | `members.what_you_do` | Similar concept |
| `website` | `members.website` | Direct mapping |
| `location` | `members.where_location` | Direct mapping |
| `social_links` (JSONB) | `social_links` table | Normalized for agent processing |
| `profile_image_url` | `members.profile_photo_url` | Direct mapping |
| `is_public` | Not in mock | All profiles public in POC |
| `membership_status` | Not in mock | Not needed for POC |
| `created_at` | `members.created_at` | Direct mapping |
| `updated_at` | `members.updated_at` | Direct mapping |

### Production Relationships Referenced

The production `creator_profiles` table has relationships with:
- `prompt_responses` - We include this in mock schema
- `persons` - Not in mock (separate feature)
- `creator_traits` - Similar to our skills/interests/inspirations
- `projects` - Not in mock (separate feature)
- Various other tables for projects, comments, likes, etc.

For the POC, we focus only on profile optimization features and exclude project management, social features, and membership management.
...
## Production Schema Alignment Notes

**For Production Integration:**

When integrating with the actual White Rabbit production database, the following adaptations will be needed:

1. **Table Name**: Change `members` to `creator_profiles`
2. **Authentication**: Add Clerk integration (`clerk_user_id`, `clerk_email`)
3. **Social Links**: Convert from normalized table to JSONB array format
4. **Traits**: Verify if `creator_traits` table structure matches our skills/interests/inspirations approach
5. **Prompt Responses**: Verify `prompt_responses` table structure matches our schema
6. **Field Mapping**: Update field names to match production (e.g., `what_you_do` → `bio` or `role`)
7. **Remove POC-specific tables**: `conversation_history` and `profile_completeness` may need different storage strategy
8. **Membership Status**: Add logic to filter by active membership tiers if needed

**CRITICAL: Missing Schema Information**

The provided schema only includes the `creator_profiles` table, but the membership form clearly shows additional data that must be stored elsewhere. We need the full schema for:

1. **`prompt_responses` table** - REQUIRED
   - Based on membership form, this should store:
   - "What is your current focus?" (current_focus)
   - "What do you need to realize your vision?" (needs_for_vision)
   - "What motivates you?" (motivations)
   - Must have: `creator_profile_id`, `prompt_key`, `response_text`, timestamps
   - Referenced in production schema: `prompt_responses.creator_profile_id → creator_profiles.id`

2. **`creator_traits` table** - REQUIRED
   - Based on membership form, this should store:
   - Skills: "What are you good at?"
   - Interests: "What are you passionate or curious about?"
   - Inspirations: "What people, books, or ideas inspire you?"
   - Must have: `creator_profile_id`, `trait_type`, `trait_value`, timestamps
   - Referenced in production schema: `creator_traits.creator_profile_id → creator_profiles.id`
   - Need to know: Is this a single table with type field, or separate tables?

3. **Phone Number Storage** - MISSING
   - Membership form shows: "Phone Number (Required for front-door check-in)"
   - NOT present in `creator_profiles` schema
   - Where is this stored? Separate table? Clerk user metadata?

4. **Social Links Structure** - UNCLEAR
   - Production uses: `social_links` as JSONB array
   - Need to know exact structure: `[{platform: "linkedin", url: "..."}]` or just `["url1", "url2"]`?
   - How is platform identification handled?

**Action Items:**

□ Request complete schema for `prompt_responses` table
□ Request complete schema for `creator_traits` table  
□ Clarify where phone numbers are stored
□ Clarify social_links JSONB structure
□ Verify if there are other tables storing membership form data

**Impact on POC:**

Until we receive the complete schema, our mock database schema makes reasonable assumptions:
- `prompt_responses` table with prompt_key/response_text structure
- Separate normalized tables for skills, interests, inspirations
- Phone number in main members table
- Separate social_links table for agent processing

These assumptions may need adjustment once we see the actual production schema.

**POC Simplifications:**

The mock schema intentionally simplifies:
- No membership tier logic
